...
#     try:
#         ds = imagefile
#         print(len(ds.pixel_array.shape))
...
def output_imgdict(imagefile):
    ...
    try:
        ds = imagefile
        if len(ds.pixel_array.shape) == 4:  # format: nframes, nrow, ncol, 3 (color channels)
            nframes = ds.pixel_array.shape[0]
            maxframes = nframes * 3
        elif len(ds.pixel_array.shape) == 3:  # format: nframes, nrow, ncol
            nframes = ds.pixel_array.shape[0]
        ...
        for counter in range(0, maxframes, 3):
            ...
            if len(ds.pixel_array.shape) == 4:
                frames = [ds.pixel_array[k, :, :, :], ds.pixel_array[m, :, :, :], ds.pixel_array[o, :, :, :]]

                for idx, frame in enumerate(frames):
                    ArrayDicom[:, :] = ybr2gray(frame[:, :, 0], frame[:, :, 1], frame[:, :, 2])
                    ArrayDicom[0:int(nrow / 10), 0:int(ncol)] = 0  # blanks out name
                    ArrayDicom.clip(0)
                    nrowout = nrow
                    ncolout = ncol
                    x = int((counter + idx + 1) / 3)
                    imgdict[x] = resize(ArrayDicom, (nrowout, ncolout))
            elif len(ds.pixel_array.shape) == 3:
                ArrayDicom[:, :] = ds.pixel_array[counter, :, :]
                ArrayDicom[0:int(nrow / 10), 0:int(ncol)] = 0  # blanks out name
                ArrayDicom.clip(0)
...
def create_imgdict_from_dicom(directory, filename):
    ...
    if ("NumberOfFrames" in dir(ds)) and (ds.NumberOfFrames > 1):
        ...
        if os.path.exists(outrawfile):
            ds = dicom.dcmread(outrawfile, force=True)
            print("entering into output_imgdict")
            imgdict = output_imgdict(ds)
        else:
            print(outrawfile, "missing")
    print("image dict = ", imgdict)
    return imgdict
