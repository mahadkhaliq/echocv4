...
def extract_area_psax(video, study, outer_segs, inner_segs, psax_areas, x_scale, y_scale, nrow, ncol):
...
def compute_lvmi(dicomDir, videofile, lvlength, hr, ft, window, x_scale, y_scale, nrow, ncol):
    ...
    print(psax_inner_segs)
    psax_outer_areas = extract_areas(psax_outer_segs)
    psax_inner_areas = extract_areas(psax_inner_segs)
    outer_area = pd.DataFrame(psax_outer_areas)[0].rolling(window=4, center=True).median().fillna(method='bfill').fillna(method='ffill').tolist()
    inner_area = pd.DataFrame(psax_inner_areas)[0].rolling(window=4, center=True).median().fillna(method='bfill').fillna(method='ffill').tolist()
    massmin = 25
    massmax = 500
    masslist = []
    for i in range(0, len(psax_outer_areas), int(window/2)):
        ...
        if (end - start) > int(0.8*window):
            ...
            outer_area_final, inner_area_final = extract_area_psax(videofile,
                                                                   dicomDir,
                                                       psax_outer_segs_window, 
                                                       psax_inner_segs_window,
                                                       inner_area_window, x_scale, y_scale, nrow, ncol)
            t = np.sqrt((outer_area_final + inner_area_final)/np.pi) - \
            np.sqrt((inner_area_final)/np.pi)
            mass = 1.05*((5/6)*(outer_area_final+inner_area_final)*(l + t) - \
...
def extract_area_l_scaled(video, directory,lv_segs, la_segs, la_areas,
                          lv_areas, x_scale, y_scale, nrow, ncol, hr):
    rows, cols = nrow, ncol
    ...
    return len(x_la)*x_scale**2, l_la*x_scale, \
           len(x_lveda)*x_scale**2, l_lveda*x_scale, \
           len(x_lvesa) * x_scale ** 2, l_lvesa * x_scale,  hr
...
def compute_la_lv_volume(dicomDir, videofile, hr, ft, window, x_scale, y_scale,
    ...
    lv_areas = pd.DataFrame(lv_areas)[0].rolling(window=4, center=True).median().fillna(method='bfill').fillna(method='ffill').tolist()
    la_areas = extract_areas(la_segs)
    la_areas = pd.DataFrame(la_areas)[0].rolling(window=4, center=True).median().fillna(method='bfill').fillna(method='ffill').tolist()
    eflist = []
    efmax, efmin = 0.8, 0.1
    ...
    for i in range(0, len(la_areas), int(window/2)):
        ...
        if (end - start) > int(0.8*window):
            ...
            try:
                la_a, la_l, lveda_a, lveda_l, lvesa_a, lvesa_l, hr = \
                extract_area_l_scaled(dicomDir, videofile, lv_segs_window, la_segs_window,
                                      la_areas_window, lv_areas_window, x_scale, y_scale, nrow, ncol, hr)
...
def main():
    ...
    viewfile = "results_dicomsample.csv"  # Updated to CSV
    ...
    probthresh = 0.000005  # Arbitrary choice of "probability" threshold for view classification

    # Dynamically create viewdict based on CSV header
    with open(viewfile, mode ='r') as file:
        csvFile = csv.reader(file)
        header = next(csvFile)
        viewdict = {col.replace('prob_', ''): col for col in header if col.startswith('prob_')}

    # Reading the main CSV file and populate view lists
    with open(viewfile, mode ='r') as file:
        csvFile = csv.reader(file)
        ...
        for line in csvFile:
            #dicomdir = line[0]
            filename = line[0]
            for view, col in viewdict.items():
                idx = header.index(col)
                if float(line[idx]) > probthresh:
                    if view.endswith('_pap'):
 ...
 #                   elif view.startswith('plax'):
 #                       viewlist_plax.append(filename)
                    elif view.startswith('psax'):
                        viewlist_plax.append(filename)
...
#    print(viewlist_a2c, viewlist_a4c, viewlist_psax)
    ...
    for videofile in viewlist_a4c + viewlist_a2c:
        ...
        compute_la_lv_volume(dicomdir, videofile, hr, ft, window, x_scale, 
                             y_scale, nrow, ncol, view)

        compute_la_lv_volume("dicomsample", "Image-43.dcm", 52,33,34, 0.0440723, 0.0440723, 600, 800, 'a4c')
