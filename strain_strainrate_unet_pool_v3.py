    ...
    if os.path.exists(segmentDir):
        ...
        for fileName in allfiles:
            if "la.npy" in fileName:
                ...
                print(filePrefix)
                try:
                    # Replace outputcropped function to accept fileName
                    #output = outputcropped(fileName, view)
                    output = outputcropped(filePrefix, view)    
                    print("output:")
                    print(output)
                    if output == 1:
...
def outputStrainpool((rawDir, tmpDir, fileName, scorethresh)):  # fileName is now an argument
    videoFile = fileName  # Now fileName is defined
    print(videoFile)
    try:
        ft, hr, nrow, ncol, x_scale, y_scale = extractmetadata(rawDir, videoFile) 
        print(ft, hr, nrow, ncol, x_scale, y_scale)
        if not (ft is None or hr is None or x_scale is None or y_scale is None):
...
time.sleep(5)
scorethresh = 0.85
    # Initialize based on file names, not frame numbers
measureDicta4c = initializeStrain("a4c")
measureDicta2c = initializeStrain("a2c")
measureDict = measureDicta4c.copy()  # Start with a copy of measureDicta4c
measureDict.update(measureDicta2c)  # Update the copy with the contents of measureDicta2c

    
pool = Pool()                         
#job_args = [(rawDir, tmpDir, fileName, scorethresh) for fileName in measureDict.keys()]
job_args = [(rawDir, tmpDir, fileName, scorethresh) for fileName in measureDict.keys()]
